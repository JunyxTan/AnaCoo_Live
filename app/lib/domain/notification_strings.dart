...
/// Copy for scheduled notifications, in the three UI languages.
///
...
class NotificationStrings {
  ...
  String get _lang => const {'en', 'zh', 'ms'}.contains(languageCode) ? languageCode : 'en';

  String _pick(String en, String zh, String ms) =>
      switch (_lang) { 'zh' => zh, 'ms' => ms, _ => en };
  ...
  String get _intlLocale => switch (_lang) { 'zh' => 'zh', 'ms' => 'ms', _ => 'en' };

  // -------------------------------------------------------------- reminders

  String reminderTitle(AppointmentType type) => switch (type) {
        AppointmentType.dropOff =>
          _pick('Drop-off coming up', '即将送件', 'Penghantaran akan tiba'),
        AppointmentType.collection =>
          _pick('Collection coming up', '即将取件', 'Pengambilan akan tiba'),
      };
  ...
  String reminderBody({
    ...
    final rush = isRush ? _pick(' (rush)', '（加急）', ' (segera)') : '';
    return _pick(
  ...
  String get agendaTitle =>
      _pick("Today's schedule", '今日安排', 'Jadual hari ini');
  ...
  String get agendaEmpty =>
      _pick('Today: nothing booked.', '今天：没有预约。', 'Hari ini: tiada tempahan.');

  // --------------------------------------------------------------- overdue

  String get overdueTitle =>
      _pick('Collection overdue', '取件逾期', 'Pengambilan tertunggak');
  ...
  String get readyNudgeTitle =>
      _pick('Still waiting for pickup', '仍未取件', 'Masih menunggu diambil');
